...
- `get_preview_client_result`: an explicit single-client render or render-diagnostic drill-down.
...
Do not fetch details for clean or unrequested checks. Use `get_preview_client_result` only for a client the user explicitly asks about or for explicit render diagnostics; never fan out across all clients. Client drill-down may summarize status, metadata, errors, and available screenshot asset keys or links, but do not visually interpret screenshots. Treat keys in `screenshots` as opaque API-provided asset names and never infer portrait, landscape, or another display orientation from a key or from client metadata.
